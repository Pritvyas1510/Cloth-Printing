...
const CancelOrder = () => {
  ...
  const statusSteps = ["processing", "design", "shipped", "delivered","cancel"];
  ...
  const fetchOrdersAndProducts = async () => {
    try {
      ...
      // Filter only orders with "cancelled" status
  ...
  if (orders.length === 0) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-gray-600 text-base">No cancelled orders found.</p>
      </div>
    );
  }
        ...
        {orders.map((order) => {
          const currentStepIndex = statusSteps.indexOf(
            order.status.toLowerCase() === "cancelled"
              ? "processing" // Assume cancellation occurred at processing stage
            ...
            >
              ...
              <div className="relative mb-6">
                <div className="text-xs text-gray-500 font-medium mb-1">
                  Cancelled on: {getExpectedArrival(order.createdAt)}
                </div>
                <div className="flex items-center">
                  <div className="w-full h-2 bg-gray-200 rounded-full relative overflow-hidden">
                    <div
                      className="h-2 bg-gradient-to-r from-red-500 to-red-700 rounded-full absolute transition-all duration-700 ease-in-out"
                      style={{
                        width: `${
